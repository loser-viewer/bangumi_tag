...
const CacheUtil_bg = {
  ...
  _generateKey: function(type, identifier) {
    if (typeof identifier === 'object' && identifier !== null) {
      try {
        return `${type}_${JSON.stringify(Object.keys(identifier).sort().reduce((obj, key) => {
  ...
  get: function(type, identifier) {
    ...
    if (entry && Date.now() < entry.expiry) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [缓存工具] 命中缓存: ${key.substring(0, 80)}...`);
      }
      return Promise.resolve(entry.value);
    } else if (entry) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [缓存工具] 缓存过期: ${key.substring(0, 80)}...`);
      }
  ...
  set: function(type, identifier, valuePromise, customTtl) {
    ...
    return valuePromise.then(value => {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [缓存工具] 设置缓存: ${key.substring(0, 80)}... (TTL: ${ttlToUse / 1000}s)`);
      }
...
const PrefetchCache_bg = {
  ...
  get: function(url) {
    ...
    if (entry) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 过期或无效: ${url}`);
      }
  ...
  set: function(url, htmlPromise) {
    if (WidgetConfig_bg.DEBUG_LOGGING) {
      console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 开始预取并设置Promise: ${url}`);
    }
      ...
      if (currentEntry === entry) {
        ...
        htmlPromise.catch(() => {
          if (this.prefetchedHtml.get(url) === entry) {
            this.prefetchedHtml.delete(url);
            if (WidgetConfig_bg.DEBUG_LOGGING) {
              console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 预取失败后删除条目: ${url}`);
            }
          }
        });
      }
    ...
    if (this.prefetchedHtml.size > WidgetConfig_bg.MAX_PREFETCHED_PAGES) {
      ...
      if (oldestKey) {
        ...
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 清理最旧条目: ${oldestKey}`);
        }
      }
    }
  ...
  fetchAndCacheHtml: function(url, headers) {
    ...
    if (existingEntry && (existingEntry.inProgress || (Date.now() - existingEntry.timestamp < WidgetConfig_bg.PREFETCH_CACHE_TTL_MS))) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 使用现有预取Promise: ${url}`);
      }
    ...
    const newHtmlPromise = fetchWithRetry_bg(url, { headers }, 'get', false, WidgetConfig_bg.HTTP_RETRIES)
      .then(response => {
        ...
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 预取成功，获得HTML: ${url}`);
        }
        return response.data;
      })
      .catch(err => {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [预取缓存] 预取网络请求失败 ${url}: ${err.message}`);
        }
        throw err;
      });
  ...
  while (attempts <= maxRetries) {
    try {
      ...
      if (!isTmdb) {
        ...
        if (url.startsWith(WidgetConfig_bg.BGM_BROWSE_URL) || url.startsWith(WidgetConfig_bg.BGM_API_BASE_URL) || url.startsWith(WidgetConfig_bg.BGM_BASE_URL)) {
...
function calculateContentTTL_bg(identifier, context) {
  ...
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 传入标识符:`, identifier);
  }
  ...
  if (year && year !== "" && month && month !== "" && month !== 'all') {
    if (isEarlySeason_bg(year, month, currentDate)) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用季度早期TTL for ${year}-${month}`);
      }
      ...
      if (monthsSinceSeasonStart > 6) {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用存档TTL for ${year}-${month}`);
        }
        return WidgetConfig_bg.TTL_ARCHIVE_MS;
      }
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用季度后期TTL for ${year}-${month}`);
      }
    ...
    if (parseInt(year, 10) < currentDate.getFullYear() - 1) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用存档TTL for year ${year}`);
      }
      return WidgetConfig_bg.TTL_ARCHIVE_MS;
    }
    if (WidgetConfig_bg.DEBUG_LOGGING) {
      console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用排行TTL for year ${year}`);
    }
    return WidgetConfig_bg.TTL_RANK_MS;
  }

  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TTL计算] 使用默认排行TTL`);
  }
  return WidgetConfig_bg.TTL_RANK_MS;
}
...
function normalizeTmdbQuery_bg(query) {
  ...
  return query.toLowerCase().trim().replace(/[\\[\\]【】（）()「」『』:：\\-－_,\\.\\・]/g, ' ').replace(/\\s+/g, ' ').trim();
}
...
function populateItemFromTmdbFullDetail_bg(itemRef, tmdbDetail) {
  ...
  if (tmdbDetail.id && !itemRef.tmdb_id) {
    itemRef.tmdb_id = String(tmdbDetail.id);
  }
...
function scoreTmdbResult_bg(result, query, validYear, searchMediaType, originalTitle, chineseTitle, isLikelyMovieOrShort = false, isShortFilm = false, scoringOverride = null) {
  const bgmItemData = scoringOverride || {
...
function generateTmdbSearchQueries_bg(originalTitle, chineseTitle, listTitle) {
  ...
  if (queriesToProcess.length > WidgetConfig_bg.MAX_TOTAL_TMDB_QUERIES_TO_PROCESS) {
    if (WidgetConfig_bg.DEBUG_LOGGING) {
      console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB搜索] 查询词过多 (${queriesToProcess.length}), 截断为 ${WidgetConfig_bg.MAX_TOTAL_TMDB_QUERIES_TO_PROCESS} 个`);
    }
...
async function searchTmdb_bg(originalTitle, chineseTitle, listTitle, searchMediaType = CONSTANTS_bg.MEDIA_TYPES.TV, year = '', isLikelyMovieOrShort = false, isShortFilm = false, bgmItemDataForScoringOverride = null) {
  ...
  return CacheUtil_bg.cachedOrFetch(CONSTANTS_bg.CACHE_KEYS.TMDB_SEARCH, cacheKeyParams, async () => {
    ...
    if (validYear && stage1QuerySource) {
      ...
      if (preciseQueryText) {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB搜索S1] ${searchIdentifierForLog} Query: "${preciseQueryText}", Year: ${validYear}`);
        }
        ...
        try {
          ...
          if (results?.length > 0) {
            for (const result of results) {
              ...
              if (searchMediaType === CONSTANTS_bg.MEDIA_TYPES.TV &&
                  !(result.genre_ids && result.genre_ids.includes(CONSTANTS_bg.TMDB_ANIMATION_GENRE_ID))) {
                continue;
              }

              let score = scoreTmdbResult_bg(
                result,
                preciseQueryText,
                validYear,
                searchMediaType,
          ...
          if (WidgetConfig_bg.DEBUG_LOGGING) {
            console.error(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB搜索S1] ${searchIdentifierForLog} API call error:`, e.message);
          }
      ...
      try {
        ...
        if (searchResults?.length > 0) {
          for (const result of searchResults) {
            ...
            const score = scoreTmdbResult_bg(
              result,
              query,
              validYear,
              searchMediaType,
...
function parseBangumiListItems_bg(htmlContent) {
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM列表解析_V2_DEBUG] 传入HTML内容 (前500字符): ${typeof htmlContent === 'string' ? htmlContent.substring(0, 500) : 'HTML内容非字符串或为空'}`);
  }
  ...
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM列表解析_V2_DEBUG] 使用选择器 "${listItemsSelector}" 找到 ${$listItems.length} 个列表项元素。`);
  }
  ...
  $listItems.each((index, element) => {
    ...
    if (subjectId && subjectId.startsWith('item_')) {
      ...
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM列表解析_V2_DEBUG] 无法解析条目ID for item at index ${index}:`, $item.find('h3 a.l').text() || '未知条目');
      }
      return;
    }
    ...
    if (!detailLink || !detailLink.trim()) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM列表解析_V2_DEBUG] 条目 "${title}" (ID: ${subjectId}) 没有有效的详情链接，已跳过。`);
      }
      return;
    }
...
if (!title) {
  ...
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM列表解析_V2_DEBUG] 函数结束，解析到 ${pendingItems.length} 个潜在条目。`);
  }
...
async function processTmdbFullDetailQueue_bg() {
  ...
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB完整详情队列] 处理批次大小: ${currentBatch.length}`);
  }
  ...
  const promises = currentBatch.map(async (task) => {
    ...
    try {
      if (!WidgetConfig_bg.FETCH_FULL_TMDB_DETAILS) {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB完整详情队列] 警告: TMDB ID ${tmdbId} 的任务在队列中，但 FETCH_FULL_TMDB_DETAILS 为 false，已跳过。`);
        }
        return;
      }
      ...
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB完整详情队列] 处理 TMDB ID ${tmdbId} 失败:`, e.message, e.stack?.substring(0,100));
      }
    }
  });
...
async function integrateTmdbDataToItem_bg(baseItem, tmdbResult, tmdbSearchType, bgmOriginalYear, isLikelyMovieOrShort, isShortFilm) {
  if (WidgetConfig_bg.DEBUG_LOGGING) {
    console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB集成] BGM "${(baseItem.title).substring(0,30)}..." (BGM ID: ${baseItem.bgm_id}, BGM Year: ${bgmOriginalYear || 'N/A'}, isShort: ${isShortFilm}) -> TMDB "${(tmdbResult.title||tmdbResult.name||'').substring(0,30)}..." (ID:${tmdbResult.id}, Year: ${(tmdbResult.release_date || tmdbResult.first_air_date)?.substring(0,4) || 'N/A'})`);
  }
  ...
  if (isShortFilm) {
    ...
    if (yearMismatchForShortFilm) {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB集成] 短片年份不匹配 (BGM: ${bgmYear}, TMDB: ${tmdbYear}). 仅记录 TMDB ID (${tmdbResult.id}) for BGM ID ${baseItem.bgm_id}.`);
      }
...
async function getBangumiDetailCover_bg(subjectId, subjectDetailUrl, bangumiAccessToken = null, bangumiCookie = null) {
  ...
  return CacheUtil_bg.cachedOrFetch(CONSTANTS_bg.CACHE_KEYS.BGM_DETAIL_COVER, cacheKeyParams, async () => {
    if (WidgetConfig_bg.DEBUG_LOGGING) {
      console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM详情封面] 尝试获取 BGM ID ${subjectId} 的高清封面从 ${subjectDetailUrl}, HasToken: ${!!bangumiAccessToken}`);
    }
    ...
    try {
      ...
      if (bPoster && !bPoster.startsWith('http') && bPoster.includes('lain.bgm.tv')) {
        ...
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM详情封面] BGM ID ${subjectId} 的封面路径非预期相对路径: ${bPoster}`);
        }
        return null;
      }
      ...
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.warn(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM详情封面] 获取 BGM ID ${subjectId} 封面失败: ${e.message}`);
      }
      return null;
    }
...
async function getBgmdMap_bg() {
  ...
  return CacheUtil_bg.cachedOrFetch(
    ...
    async () => {
      if (WidgetConfig_bg.DEBUG_LOGGING) {
        console.log(`${logPrefix} Fetching and processing bgmd index.json from ${WidgetConfig_bg.BGMD_INDEX_URL}`);
      }

      try {
        const response = await fetchWithRetry_bg(
          WidgetConfig_bg.BGMD_INDEX_URL,
          {
            headers: {
        ...
        if (typeof data === "string") {
          try {
            ...
            console.error(`${logPrefix} index.json 字符串解析失败:`, e.message, String(data).substring(0, 300));
            return new Map();
          }
        }

        if (!data || !data.bangumis || !Array.isArray(data.bangumis)) {
          console.error(`${logPrefix} Failed to fetch or parse bgmd index.json: Invalid data structure. Response:`, typeof data === "string" ? data.substring(0, 500) : JSON.stringify(data).substring(0, 500));
          return new Map();
        }

        const bangumiArray = data.bangumis;
        ...
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${logPrefix} Successfully processed ${bgmdMap.size} entries from bgmd index.json into map.`);
        }
...
async function fetchItemDetails_bg(pendingItem, categoryHint, rankingContext = {}, bangumiAccessToken = null, bangumiCookie = null) {
      ...
      if (detailHtmlResponse?.data) {
        ...
        if (!determinedByInfoboxTV && !determinedByPageTagTV) {
          if (isTheatricalMovie) {
            ...
            isShortFilm = false;
            if (hasShortFilmTag) isShortFilm = true;
          } else if (isOvaOrOad) {
  ...
  if (WidgetConfig_bg.BGM_USE_BGMD_INDEX) {
    try {
      ...
      if (bgmdEntry && bgmdEntry.tmdbId && bgmdEntry.tmdbType) {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.log(`${logPrefixDetails} Found mapping in BGMD for BGM ID ${pendingItem.id} -> TMDB ID ${bgmdEntry.tmdbId} (Type: ${bgmdEntry.tmdbType})`);
        }

        item.tmdb_id = String(bgmdEntry.tmdbId);
        item.mediaType = bgmdEntry.tmdbType;
        item.type = "tmdb";
        item.id = String(bgmdEntry.tmdbId);
        item.link = null;

        if (!item.title || item.title.trim() === "") {
          item.title = cTitleFromBgmDetail || oTitle || bgmdEntry.tmdbName || bgmdEntry.tmdbOriginalName || item.title;
        }

        if (!WidgetConfig_bg.FETCH_TMDB_DETAIL_FOR_BGMD_HIT) {
          if (WidgetConfig_bg.DEBUG_LOGGING) {
            console.log(`${logPrefixDetails} BGMD hit for BGM ID ${pendingItem.id}, using lightweight mapped result without TMDB detail fetch.`);
          }
          return item;
        }
        ...
        try {
            ...
            async () => {
              const response = await fetchWithRetry_bg(
                `/${bgmdEntry.tmdbType}/${bgmdEntry.tmdbId}`,
                {
                  params: {
                    language: "zh-CN",
                    append_to_response: WidgetConfig_bg.TMDB_APPEND_TO_RESPONSE
                  }
                },
                "get",
                true,
          ...
          if (tmdbDetail && tmdbDetail.id) {
            if (WidgetConfig_bg.DEBUG_LOGGING) {
              console.log(`${logPrefixDetails} Successfully fetched TMDB full detail for mapped ID ${bgmdEntry.tmdbId} (BGM ID ${pendingItem.id}).`);
            }
            ...
            if (!item.title || item.title === (cTitleFromBgmDetail || oTitle)) {
              item.title =
                item.tmdb_preferred_title ||
                tmdbDetail.title ||
                tmdbDetail.name ||
                bgmdEntry.tmdbName ||
                bgmdEntry.tmdbOriginalName ||
                item.title;
            }

            item.posterPath = tmdbDetail.poster_path
              ? `https://image.tmdb.org/t/p/w500${tmdbDetail.poster_path}`
              : item.posterPath;

            item.backdropPath = tmdbDetail.backdrop_path
            ...
            if (WidgetConfig_bg.DEBUG_LOGGING) {
              console.log(`${logPrefixDetails} [BGMD Path] Processing complete for BGM ID ${pendingItem.id}. Final ID: ${item.id}, Type: ${item.type}, Title: "${String(item.title).substring(0, 30)}..."`);
            }
          } else if (WidgetConfig_bg.DEBUG_LOGGING) {
            console.warn(`${logPrefixDetails} BGMD hit but TMDB detail response invalid for mapped ID ${bgmdEntry.tmdbId} (BGM ID ${pendingItem.id}). Returning lightweight item.`);
          }

          return item;
        } catch (e) {
          console.warn(`${logPrefixDetails} BGMD hit but TMDB detail fetch failed for mapped ID ${bgmdEntry.tmdbId} (BGM ID ${pendingItem.id}). Returning lightweight item. Error: ${e.message}`);
          return item;
        }
      } else if (bgmdEntry) {
        if (WidgetConfig_bg.DEBUG_LOGGING) {
          console.warn(`${logPrefixDetails} BGMD entry for BGM ID ${pendingItem.id} found but invalid (missing tmdbId or tmdbType). Falling back to search. Entry: ${JSON.stringify(bgmdEntry)}`);
        }
      }
    } catch (bgmdError) {
  ...
  if (tmdbRes?.id) {
    ...
    if (WidgetConfig_bg.DEBUG_LOGGING) {
      console.log(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [TMDB过滤] TMDB 未命中，丢弃条目: BGM ID ${pendingItem.id}, title="${pendingItem.titleFromList}"`);
    }
    return null;
  }
  ...
  for (let i = 0; i < pendingItems.length; i += WidgetConfig_bg.MAX_CONCURRENT_DETAILS_FETCH) {
    ...
    settledResults.forEach(sr => {
      if (sr.status === 'fulfilled' && sr.value) {
        ...
        console.error(`${CONSTANTS_bg.LOG_PREFIX_GENERAL} [BGM页面处理] 一个条目详情Promise被拒绝:`, sr.reason?.message);
      }
    });
  }
